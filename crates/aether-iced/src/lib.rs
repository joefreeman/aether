...
/// Run the native GUI client to completion. `project`/`file` are the (optional) CLI positionals and
/// `version` is the handshake version string. iced owns the main thread and manages its own tokio
/// runtime, so unlike the terminal client this is a synchronous call (not awaited on a runtime the
/// caller provides).
pub fn run(project: Option<String>, file: Option<String>, version: String) -> anyhow::Result<()> {
