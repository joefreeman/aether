...
//! there). This shell consumes the ordered [`Inbound`] stream from a single place — the run loop's
//! select arm — which is what carries the wire-order delivery contract through to processing.
